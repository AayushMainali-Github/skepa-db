...
fn test_update_where_like() {
    ...
    db.execute(r#"insert into users values (1, "ram", 20)"#)
        .unwrap();
    db.execute(r#"insert into users values (2, "rom", 30)"#)
        .unwrap();
    db.execute(r#"insert into users values (3, "alice", 40)"#)
        .unwrap();
...
fn test_update_where_parentheses() {
    ...
    db.execute(r#"insert into users values (1, 20, "ny")"#)
        .unwrap();
    db.execute(r#"insert into users values (2, 16, "la")"#)
        .unwrap();
...
fn test_delete_where_like() {
    ...
    db.execute(r#"insert into users values (2, "rom")"#)
        .unwrap();
    db.execute(r#"insert into users values (3, "alice")"#)
        .unwrap();

    let out = db
        .execute(r#"delete from users where name like "r?m""#)
        .unwrap();
...
fn test_delete_like_star_removes_all_rows() {
    ...
    let out = db
        .execute(r#"delete from users where name like "*""#)
        .unwrap();
    assert_eq!(out, "deleted 2 row(s) from users");
...
fn test_update_no_match_returns_zero() {
    ...
    let out = db
        .execute("update users set age = 21 where id = 99")
        .unwrap();
...
fn test_update_changes_multiple_rows_with_numeric_predicate() {
    ...
    assert_eq!(
        db.execute("select * from nums").unwrap(),
        "id\tv\n1\t10\n2\t0\n3\t0"
    );
}
...
fn test_update_can_set_text_empty_string() {
    ...
    let out = db
        .execute(r#"update users set name = "" where id = 1"#)
        .unwrap();
    assert_eq!(out, "updated 1 row(s) in users");
    assert_eq!(db.execute("select * from users").unwrap(), "id\tname\n1\t");
}
