...
fn test_insert_into_missing_table() {
    let mut db = test_db();
    let result = db.execute_legacy(r#"insert into users values (1, "ram")"#);
...
fn test_insert_wrong_value_count() {
    ...
    let result = db.execute_legacy(r#"insert into users values (1, "ram", "extra")"#);
    assert!(result.is_err());
...
fn test_insert_type_mismatch_int() {
    ...
    let result = db.execute_legacy(r#"insert into users values ("abc", "ram")"#);
    assert!(result.is_err());
...
fn test_update_multiple_columns() {
    ...
    let out = db
        .execute_legacy(r#"update users set name = "ravi", age = 25 where id eq 1"#)
        .unwrap();
...
fn test_update_where_like() {
    ...
    db.execute_legacy(r#"insert into users values (1, "ram", 20)"#)
        .unwrap();
    db.execute_legacy(r#"insert into users values (2, "rom", 30)"#)
        .unwrap();
    db.execute_legacy(r#"insert into users values (3, "alice", 40)"#)
        .unwrap();

    let out = db
        .execute_legacy(r#"update users set age = 99 where name like "r?m""#)
        .unwrap();
...
fn test_update_where_is_null() {
    ...
    let msg = db
        .execute_legacy(r#"update users set city = "ny" where city is null"#)
        .unwrap();
    assert_eq!(msg, "updated 1 row(s) in users");
    let out = db
        .execute_legacy("select * from users order by id asc")
        .unwrap();
...
fn test_update_where_in() {
    ...
    db.execute_legacy(r#"insert into users values (1, "a")"#)
        .unwrap();
    db.execute_legacy(r#"insert into users values (2, "b")"#)
        .unwrap();
    db.execute_legacy(r#"insert into users values (3, "c")"#)
        .unwrap();

    let msg = db
        .execute_legacy(r#"update users set city = "x" where id in (1,3)"#)
        .unwrap();
...
fn test_delete_where_in() {
    ...
    let msg = db
        .execute_legacy("delete from users where id in (2,3)")
        .unwrap();
...
fn test_where_in_type_mismatch_errors() {
    ...
    db.execute_legacy(r#"insert into users values (1, "a")"#)
        .unwrap();
    let err = db
        .execute_legacy(r#"select * from users where id in ("x","y")"#)
        .unwrap_err();
...
fn test_update_where_and() {
    ...
    let msg = db
        .execute_legacy(r#"update users set city = "x" where age = 20 and city = "ny""#)
        .unwrap();
    assert_eq!(msg, "updated 1 row(s) in users");
    let out = db
        .execute_legacy("select * from users order by id asc")
...
fn test_delete_where_or() {
    ...
    let msg = db
        .execute_legacy(r#"delete from users where city = "ny" or city = "sf""#)
        .unwrap();
    assert_eq!(msg, "deleted 2 row(s) from users");
    let out = db.execute_legacy("select * from users").unwrap();
    assert_eq!(out, "id\tcity\n2\tla");
...
fn test_update_where_parentheses() {
    ...
    db.execute_legacy(r#"insert into users values (1, 20, "ny")"#)
        .unwrap();
    db.execute_legacy(r#"insert into users values (2, 16, "la")"#)
        .unwrap();
    ...
    let msg = db
        .execute_legacy(
            r#"update users set city = "x" where (age gte 18 and city = "ny") or id = 2"#,
        )
        .unwrap();
    assert_eq!(msg, "updated 2 row(s) in users");
    let out = db
...
fn test_update_unknown_set_column_errors() {
    ...
    let result = db.execute_legacy(r#"update users set age = 20 where id = 1"#);
    assert!(result.is_err());
    assert!(
        result
            .unwrap_err()
            .to_lowercase()
            .contains("unknown column 'age' in update")
    );
}
...
fn test_update_type_mismatch_errors() {
    ...
    let result = db.execute_legacy(r#"update users set age = "bad" where id = 1"#);
    assert!(result.is_err());
    assert!(result.unwrap_err().to_lowercase().contains("expected int"));
}
...
fn test_delete_where_like() {
    ...
    db.execute_legacy(r#"insert into users values (2, "rom")"#)
        .unwrap();
    db.execute_legacy(r#"insert into users values (3, "alice")"#)
        .unwrap();

    let out = db
        .execute_legacy(r#"delete from users where name like "r?m""#)
        .unwrap();
...
fn test_delete_unknown_column_errors() {
    ...
    let result = db.execute_legacy("delete from users where age = 10");
    assert!(result.is_err());
    assert!(
        result
            .unwrap_err()
...
fn test_delete_no_match_returns_zero() {
    ...
    let out = db
        .execute_legacy("delete from users where id = 99")
        .unwrap();
...
fn test_delete_numeric_gt() {
    let mut db = test_db();
    db.execute_legacy("create table nums (id int, v int)")
        .unwrap();
    ...
    let out = db.execute_legacy("delete from nums where v > 15").unwrap();
    assert_eq!(out, "deleted 2 row(s) from nums");
    assert_eq!(
        db.execute_legacy("select * from nums").unwrap(),
        "id\tv\n1\t10"
    );
}
...
fn test_delete_numeric_lte() {
    ...
    let out = db.execute_legacy("delete from nums where v <= 20").unwrap();
    assert_eq!(out, "deleted 2 row(s) from nums");
    assert_eq!(
        db.execute_legacy("select * from nums").unwrap(),
        "id\tv\n3\t30"
...
fn test_delete_like_star_removes_all_rows() {
    ...
    let out = db
        .execute_legacy(r#"delete from users where name like "*""#)
        .unwrap();
    assert_eq!(out, "deleted 2 row(s) from users");
    assert_eq!(
...
fn test_delete_like_question_single_char() {
    ...
    db.execute_legacy(r#"insert into users values (1, "a")"#)
        .unwrap();
    db.execute_legacy(r#"insert into users values (2, "bb")"#)
        .unwrap();
...
fn test_delete_text_with_gt_errors() {
    ...
    let result = db.execute_legacy("delete from users where name > 1");
    assert!(result.is_err());
    assert!(
        result
            .unwrap_err()
...
fn test_update_no_match_returns_zero() {
    ...
    let out = db
        .execute_legacy("update users set age = 21 where id = 99")
        .unwrap();
...
fn test_update_unknown_where_column_errors() {
    ...
    let result = db.execute_legacy("update users set age = 21 where missing = 1");
    assert!(result.is_err());
    assert!(
        result
            .unwrap_err()
...
fn test_update_where_like_on_int_errors() {
    ...
    let result = db.execute_legacy(r#"update users set age = 21 where age like "2*""#);
    assert!(result.is_err());
    assert!(
        result
            .unwrap_err()
...
fn test_update_changes_multiple_rows_with_numeric_predicate() {
    ...
    let out = db
        .execute_legacy("update nums set v = 0 where v >= 20")
        .unwrap();
...
fn test_update_can_set_text_empty_string() {
    ...
    let out = db
        .execute_legacy(r#"update users set name = "" where id = 1"#)
        .unwrap();
    assert_eq!(out, "updated 1 row(s) in users");
    assert_eq!(
        db.execute_legacy("select * from users").unwrap(),
...
fn test_delete_from_missing_table_errors() {
    let mut db = test_db();
    let result = db.execute_legacy("delete from users where id = 1");
    assert!(result.is_err());
    assert!(
        result
            .unwrap_err()
...
fn test_update_missing_table_errors() {
    let mut db = test_db();
    let result = db.execute_legacy("update users set id = 1 where id = 1");
    assert!(result.is_err());
    assert!(
        result
            .unwrap_err()
...
fn test_update_pk_eq_no_match_returns_zero() {
    ...
    let out = db
        .execute_legacy("update t set v = 99 where id = 999")
        .unwrap();
...
fn test_delete_pk_eq_no_match_returns_zero() {
    ...
    db.execute_legacy("create table t (id int primary key, v int)")
        .unwrap();
    db.execute_legacy("insert into t values (1, 10)").unwrap();
    let out = db.execute_legacy("delete from t where id = 999").unwrap();
...
fn test_update_same_value_still_reports_updated_row() {
    ...
    db.execute_legacy("insert into t values (1, 10)").unwrap();
    let out = db
        .execute_legacy("update t set v = 10 where id = 1")
        .unwrap();
...
fn test_delete_all_rows_using_like_star_then_insert_again() {
    ...
    db.execute_legacy(r#"insert into t values (2, "b")"#)
        .unwrap();
    db.execute_legacy(r#"delete from t where name like "*""#)
        .unwrap();
    db.execute_legacy(r#"insert into t values (3, "c")"#)
        .unwrap();
    assert_eq!(
