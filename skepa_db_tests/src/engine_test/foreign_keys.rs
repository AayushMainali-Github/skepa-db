...
fn test_foreign_key_insert_requires_parent_row() {
    let mut db = test_db();
    db.execute_legacy("create table users (id int primary key)")
        .unwrap();
    db.execute_legacy(
    ...
    .unwrap();
    let err = db
        .execute_legacy("insert into orders values (1, 99)")
        .unwrap_err();
...
fn test_foreign_key_insert_succeeds_when_parent_exists() {
    ...
    .unwrap();
    db.execute_legacy("insert into users values (1)").unwrap();
    db.execute_legacy("insert into orders values (1, 1)")
        .unwrap();
    assert_eq!(
        db.execute_legacy("select * from orders").unwrap(),
        "id\tuser_id\n1\t1"
    );
}
...
fn test_foreign_key_insert_succeeds_when_parent_unique_exists() {
    ...
    db.execute_legacy(r#"insert into users values (1, "a@x.com")"#)
        .unwrap();
    db.execute_legacy(r#"insert into orders values (1, "a@x.com")"#)
        .unwrap();
    assert_eq!(
        db.execute_legacy("select * from orders").unwrap(),
        "id\tuser_email\n1\ta@x.com"
...
fn test_foreign_key_restrict_blocks_parent_delete() {
    ...
    db.execute_legacy("insert into orders values (1, 1)")
        .unwrap();
    let err = db
        .execute_legacy("delete from users where id = 1")
        .unwrap_err();
...
fn test_foreign_key_restrict_blocks_parent_delete_with_child_secondary_index() {
    ...
    .unwrap();
    db.execute_legacy("create index on orders (user_id)")
        .unwrap();
    db.execute_legacy("insert into users values (1)").unwrap();
    db.execute_legacy("insert into orders values (1, 1)")
        .unwrap();
...
fn test_foreign_key_restrict_blocks_parent_update() {
    ...
    db.execute_legacy("insert into orders values (1, 1)")
        .unwrap();
    let err = db
        .execute_legacy("update users set id = 2 where id = 1")
        .unwrap_err();
    assert!(err.to_lowercase().contains("foreign key restrict"));
}
...
fn test_foreign_key_update_child_requires_parent() {
    ...
    db.execute_legacy("insert into orders values (1, 1)")
        .unwrap();
    let err = db
        .execute_legacy("update orders set user_id = 2 where id = 1")
        .unwrap_err();
    assert!(err.to_lowercase().contains("foreign key"));
}
...
fn test_composite_foreign_key_enforced() {
    let mut db = test_db();
    db.execute_legacy("create table parent (a int, b int, primary key(a,b))")
        .unwrap();
    db.execute_legacy(
        "create table child (id int, a int, b int, foreign key(a,b) references parent(a,b))",
    )
    .unwrap();
    db.execute_legacy("insert into parent values (1, 2)")
        .unwrap();
    db.execute_legacy("insert into child values (1, 1, 2)")
        .unwrap();
    let err = db
        .execute_legacy("insert into child values (2, 9, 9)")
        .unwrap_err();
    assert!(err.to_lowercase().contains("foreign key"));
}
...
fn test_foreign_key_on_delete_cascade_deletes_children() {
    ...
    .unwrap();
    db.execute_legacy("insert into users values (1)").unwrap();
    db.execute_legacy("insert into users values (2)").unwrap();
    db.execute_legacy("insert into orders values (1, 1)")
        .unwrap();
    db.execute_legacy("insert into orders values (2, 2)")
        .unwrap();

    db.execute_legacy("delete from users where id = 1").unwrap();
    assert_eq!(
        db.execute_legacy("select * from orders").unwrap(),
        "id\tuser_id\n2\t2"
    );
...
fn test_composite_foreign_key_on_delete_cascade() {
    ...
    db.execute_legacy("insert into parent values (1, 2)")
        .unwrap();
    db.execute_legacy("insert into parent values (3, 4)")
        .unwrap();
    db.execute_legacy("insert into child values (1, 1, 2)")
        .unwrap();
    db.execute_legacy("insert into child values (2, 3, 4)")
        .unwrap();

    db.execute_legacy("delete from parent where a = 1").unwrap();
    assert_eq!(
        db.execute_legacy("select * from child").unwrap(),
        "id\ta\tb\n2\t3\t4"
    );
}
...
fn test_fk_create_rejects_unknown_parent_table() {
    let mut db = test_db();
    db.execute_legacy("create table child (id int, p int, foreign key(p) references parent(id))")
        .unwrap_err();
}
...
fn test_fk_create_rejects_unknown_parent_column() {
    let mut db = test_db();
    db.execute_legacy("create table parent (id int primary key)")
        .unwrap();
    let err = db
        .execute_legacy(
            "create table child (id int, p int, foreign key(p) references parent(missing))",
        )
        .unwrap_err();
...
fn test_fk_create_rejects_unknown_child_column() {
    ...
    db.execute_legacy("create table parent (id int primary key)")
        .unwrap();
    let err = db
        .execute_legacy("create table child (id int, foreign key(missing) references parent(id))")
        .unwrap_err();
...
fn test_fk_create_rejects_mismatched_column_count() {
    ...
    db.execute_legacy("create table parent (a int, b int, primary key(a,b))")
        .unwrap();
    let err = db
        .execute_legacy("create table child (x int, foreign key(x) references parent(a,b))")
        .unwrap_err();
...
fn test_fk_create_requires_parent_key_or_unique() {
    let mut db = test_db();
    db.execute_legacy("create table parent (id int, v int)")
        .unwrap();
    let err = db
        .execute_legacy("create table child (id int, p int, foreign key(p) references parent(id))")
        .unwrap_err();
...
fn test_fk_create_allows_parent_unique_reference() {
    let mut db = test_db();
    db.execute_legacy("create table parent (id int, code text unique)")
        .unwrap();
    db.execute_legacy(
        "create table child (id int, code text, foreign key(code) references parent(code))",
    )
    .unwrap();
}
...
fn test_fk_restrict_parent_delete_allowed_when_unreferenced() {
    ...
    db.execute_legacy("create table p (id int primary key)")
        .unwrap();
    db.execute_legacy("create table c (id int, pid int, foreign key(pid) references p(id))")
        .unwrap();
    db.execute_legacy("insert into p values (1)").unwrap();
    db.execute_legacy("insert into p values (2)").unwrap();
    db.execute_legacy("insert into c values (1, 1)").unwrap();
    let out = db.execute_legacy("delete from p where id = 2").unwrap();
...
fn test_fk_cascade_deletes_multiple_child_rows() {
    ...
    db.execute_legacy("insert into p values (1)").unwrap();
    db.execute_legacy("insert into c values (1, 1)").unwrap();
    db.execute_legacy("insert into c values (2, 1)").unwrap();
...
fn test_fk_cascade_and_restrict_can_coexist() {
    ...
    db.execute_legacy("create table p (id int primary key)")
        .unwrap();
    db.execute_legacy(
        "create table c1 (id int, pid int, foreign key(pid) references p(id) on delete cascade)",
    )
    .unwrap();
    db.execute_legacy("create table c2 (id int, pid int, foreign key(pid) references p(id))")
        .unwrap();
    db.execute_legacy("insert into p values (1)").unwrap();
    db.execute_legacy("insert into c1 values (1, 1)").unwrap();
    db.execute_legacy("insert into c2 values (1, 1)").unwrap();
    let err = db.execute_legacy("delete from p where id = 1").unwrap_err();
...
fn test_fk_child_update_to_existing_parent_succeeds() {
    ...
    db.execute_legacy("insert into p values (2)").unwrap();
    db.execute_legacy("insert into c values (1, 1)").unwrap();
    db.execute_legacy("update c set pid = 2 where id = 1")
        .unwrap();
    assert_eq!(
...
fn test_fk_parent_update_non_key_column_allowed() {
    let mut db = test_db();
    db.execute_legacy("create table p (id int primary key, name text)")
        .unwrap();
    db.execute_legacy("create table c (id int, pid int, foreign key(pid) references p(id))")
        .unwrap();
    db.execute_legacy(r#"insert into p values (1, "a")"#)
        .unwrap();
    db.execute_legacy("insert into c values (1, 1)").unwrap();
    db.execute_legacy(r#"update p set name = "b" where id = 1"#)
        .unwrap();
    assert_eq!(
        db.execute_legacy("select * from p").unwrap(),
        "id\tname\n1\tb"
    );
}
...
fn test_fk_composite_restrict_blocks_parent_delete() {
    let mut db = test_db();
    db.execute_legacy("create table p (a int, b int, primary key(a,b))")
        .unwrap();
    db.execute_legacy("create table c (id int, a int, b int, foreign key(a,b) references p(a,b))")
        .unwrap();
    db.execute_legacy("insert into p values (1, 2)").unwrap();
    db.execute_legacy("insert into c values (1, 1, 2)").unwrap();
    let err = db.execute_legacy("delete from p where a = 1").unwrap_err();
    assert!(err.to_lowercase().contains("restrict"));
}
...
fn test_foreign_key_on_update_restrict_blocks_parent_key_update() {
    ...
    db.execute_legacy("create table p (id int primary key)")
        .unwrap();
    db.execute_legacy(
        "create table c (id int, pid int, foreign key(pid) references p(id) on update restrict)",
    )
    .unwrap();
    db.execute_legacy("insert into p values (1)").unwrap();
    db.execute_legacy("insert into c values (1, 1)").unwrap();
    let err = db
        .execute_legacy("update p set id = 2 where id = 1")
        .unwrap_err();
    assert!(err.to_lowercase().contains("restrict"));
}
...
fn test_foreign_key_on_update_cascade_updates_child_rows() {
    ...
    db.execute_legacy("create table p (id int primary key)")
        .unwrap();
    db.execute_legacy(
        "create table c (id int, pid int, foreign key(pid) references p(id) on update cascade)",
    )
    .unwrap();
    db.execute_legacy("insert into p values (1)").unwrap();
    db.execute_legacy("insert into c values (1, 1)").unwrap();
...
fn test_composite_foreign_key_on_update_cascade() {
    ...
    db.execute_legacy("insert into p values (1, 2)").unwrap();
    db.execute_legacy("insert into c values (1, 1, 2)").unwrap();
    db.execute_legacy("update p set a = 3 where a = 1").unwrap();
    assert_eq!(
        db.execute_legacy("select * from c").unwrap(),
        "id\ta\tb\n1\t3\t2"
    );
...
fn test_foreign_key_on_delete_set_null_sets_child_to_null() {
    ...
    db.execute_legacy("create table p (id int primary key)")
        .unwrap();
    db.execute_legacy(
        "create table c (id int, pid int, foreign key(pid) references p(id) on delete set null)",
    )
    .unwrap();
    db.execute_legacy("insert into p values (1)").unwrap();
    db.execute_legacy("insert into c values (1, 1)").unwrap();
...
fn test_foreign_key_on_update_set_null_sets_child_to_null() {
    ...
    db.execute_legacy("create table p (id int primary key)")
        .unwrap();
    db.execute_legacy(
        "create table c (id int, pid int, foreign key(pid) references p(id) on update set null)",
    )
    .unwrap();
    db.execute_legacy("insert into p values (1)").unwrap();
    db.execute_legacy("insert into c values (1, 1)").unwrap();
...
fn test_foreign_key_set_null_requires_nullable_child_columns() {
    ...
    db.execute_legacy("create table p (id int primary key)")
        .unwrap();
    let err = db
        .execute_legacy("create table c (id int, pid int not null, foreign key(pid) references p(id) on delete set null)")
        .unwrap_err();
...
fn test_foreign_key_on_delete_no_action_behaves_like_restrict() {
    ...
    db.execute_legacy("create table p (id int primary key)")
        .unwrap();
    db.execute_legacy(
        "create table c (id int, pid int, foreign key(pid) references p(id) on delete no action)",
    )
    .unwrap();
    db.execute_legacy("insert into p values (1)").unwrap();
    db.execute_legacy("insert into c values (1, 1)").unwrap();
...
fn test_foreign_key_on_update_no_action_behaves_like_restrict() {
    ...
    db.execute_legacy("create table p (id int primary key)")
        .unwrap();
    db.execute_legacy(
        "create table c (id int, pid int, foreign key(pid) references p(id) on update no action)",
    )
    .unwrap();
    db.execute_legacy("insert into p values (1)").unwrap();
    db.execute_legacy("insert into c values (1, 1)").unwrap();
...
fn test_foreign_key_no_action_delete_deferred_until_commit_can_be_fixed_in_tx() {
    ...
    db.execute_legacy("begin").unwrap();
    db.execute_legacy("delete from p where id = 1").unwrap();
    db.execute_legacy("delete from c where id = 1").unwrap();
    let out = db.execute_legacy("commit").unwrap();
    assert_eq!(out, "transaction committed");
    assert_eq!(db.execute_legacy("select * from p").unwrap(), "id");
    assert_eq!(db.execute_legacy("select * from c").unwrap(), "id\tpid");
...
fn test_foreign_key_insert_with_null_child_key_is_allowed() {
    ...
    db.execute_legacy("create table c (id int, pid int, foreign key(pid) references p(id))")
        .unwrap();
    db.execute_legacy("insert into c values (1, null)").unwrap();
    assert_eq!(
        db.execute_legacy("select * from c").unwrap(),
        "id\tpid\n1\tnull"
    );
...
fn test_foreign_key_update_child_to_null_is_allowed() {
    ...
    db.execute_legacy("insert into p values (1)").unwrap();
    db.execute_legacy("insert into c values (1, 1)").unwrap();
    db.execute_legacy("update c set pid = null where id = 1")
        .unwrap();
    assert_eq!(
        db.execute_legacy("select * from c").unwrap(),
        "id\tpid\n1\tnull"
...
fn test_composite_foreign_key_insert_with_partial_null_is_allowed() {
    ...
    db.execute_legacy("create table c (id int, a int, b int, foreign key(a,b) references p(a,b))")
        .unwrap();
    db.execute_legacy("insert into c values (1, null, 2)")
        .unwrap();
    assert_eq!(
        db.execute_legacy("select * from c").unwrap(),
        "id\ta\tb\n1\tnull\t2"
...
fn test_foreign_key_on_delete_cascade_propagates_to_grandchildren() {
    ...
    db.execute_legacy("insert into g values (100, 10)").unwrap();

    db.execute_legacy("delete from p where id = 1").unwrap();
    assert_eq!(db.execute_legacy("select * from c").unwrap(), "id\tpid");
    assert_eq!(db.execute_legacy("select * from g").unwrap(), "id\tcid");
}
...
fn test_foreign_key_on_delete_set_null_then_child_cascade_propagates() {
    ...
    assert_eq!(
        db.execute_legacy("select * from g").unwrap(),
        "id\tcid\n100\t10"
    );
}
...
fn test_foreign_key_on_update_cascade_propagates_to_grandchildren() {
    ...
    .unwrap();
    db.execute_legacy("insert into p values (1)").unwrap();
    db.execute_legacy("insert into c values (1)").unwrap();
    db.execute_legacy("insert into g values (100, 1)").unwrap();

    db.execute_legacy("update p set id = 2 where id = 1")
        .unwrap();
    assert_eq!(db.execute_legacy("select * from c").unwrap(), "id\n2");
    assert_eq!(
        db.execute_legacy("select * from g").unwrap(),
        "id\tcid\n100\t2"
    );
